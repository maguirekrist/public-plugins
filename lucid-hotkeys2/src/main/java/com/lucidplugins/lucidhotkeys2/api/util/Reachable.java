...
public class Reachable
{
    ...
    public static boolean canTravelInDirection(WorldArea w1, int dx, int dy, Predicate<? super WorldPoint> extraCondition)
    {
        ...
        {
            LocalPoint lp = LocalPoint.fromWorld(Static.getClient(), w1.getX(), w1.getY());
            int startX = 0;
            if (lp != null)
            {
                ...
                else
                {
                    ...
                    if (dx != 0)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if ((collisionDataFlags[checkX][x] & xFlags) != 0 || !extraCondition.test(WorldPoint.fromScene(Static.getClient(), checkX, x, w1.getPlane())))
                            {
                                return false;
                            }
                    ...
                    if (dy != 0)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if ((collisionDataFlags[x][checkY] & yFlags) != 0 || !extraCondition.test(WorldPoint.fromScene(Static.getClient(), x, checkY, Static.getClient().getLocalPlayer().getWorldLocation().getPlane())))
                            {
                                return false;
                            }
                    ...
                    if (dx != 0 && dy != 0)
                    {
                        if ((collisionDataFlags[checkX][checkY] & xyFlags) != 0 || !extraCondition.test(WorldPoint.fromScene(Static.getClient(), checkX, checkY, Static.getClient().getLocalPlayer().getWorldLocation().getPlane())))
                        {
                            return false;
                        }

                        if (w1.getWidth() == 1 && (collisionDataFlags[checkX][checkY - dy] & xFlags) != 0 && extraCondition.test(WorldPoint.fromScene(Static.getClient(), checkX, startY, Static.getClient().getLocalPlayer().getWorldLocation().getPlane())))
                        {
                            return false;
                        }
                        ...
                        if (w1.getHeight() == 1)
                        {
                            return (collisionDataFlags[checkX - dx][checkY] & yFlags) == 0 || !extraCondition.test(WorldPoint.fromScene(Static.getClient(), startX, checkY, Static.getClient().getLocalPlayer().getWorldLocation().getPlane()));
                        }
                    }
